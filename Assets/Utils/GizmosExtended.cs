...
namespace Utilities
{
	...
	/// <summary>Gizmo Extension - Static class that extends Unity's gizmo functionality.</summary>
	public static class GizmosExtended
	{
		...
		/// <summary>- Draws a 2D axis-aligned bounding box (ignores y, instead using a single y value).</summary>
		/// <param name='bounds'>- The bounds to draw.</param>
		/// <param name='color'>- The color of the bounds.</param>
		public static void DrawBounds2D(Bounds2D bounds, float y, Color color = default(Color))
		{
			Vector3
				rf = new Vector3(bounds.max.x, y, bounds.min.y),
				rb = new Vector3(bounds.max.x, y, bounds.max.y),
				lf = new Vector3(bounds.min.x, y, bounds.min.y),
				lb = new Vector3(bounds.min.x, y, bounds.max.y);

			using (new ColorScope(color))
			{
				Gizmos.DrawLine(rf, lf);
				Gizmos.DrawLine(rf, rb);
